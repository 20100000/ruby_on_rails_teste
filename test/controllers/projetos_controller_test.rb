...
class ProjetosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create projeto" do
    assert_difference('Projeto.count') do
      post projetos_url, params: { projeto: { name: @projeto.name, user_id: @projeto.user_id } }
    end
  ...
  test "should update projeto" do
    patch projeto_url(@projeto), params: { projeto: { name: @projeto.name, user_id: @projeto.user_id } }
